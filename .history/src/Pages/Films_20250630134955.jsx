...
export default function Films() {
    ...
    <ul className="mt-10 md:text-2xl mx-10">
      ...
      {(showSelected ? deferredFilms : recentFilms).map((film) => (
        ...
        <li
          key={film.id}
          className="list-decimal m-3 border p-4 rounded-xl font-normal shadow text-center bg-white space-y-2 text-size-small-medium" 
        >
        
          <NavLink to={`/category/${category.id}`} >{film.name} {film.date >= "" ? `- ${film.date}`: ``}</NavLink>
