...
export default function Films() {
    ...
    <h1 className=" font-semibold text-xl md:text-xl">Избранные:</h1>
      <ul className="mt-10 md:text-2xl mx-10 grid grid-cols-4 ">
        {favorite.map((fav) => (
            <li key={fav.id} className="flex items-center flex-col mx-2 pt-2 
            text-size-small-medium ">
            <FilmPoster className="w-40 hover:bg-indigo-100 transition-colors duration-300 cursor-pointer border rounded-xl shadow-[5px_5px_10px_rgba(0,0,0,0.2)]" filmName={fav.name} year={fav.year} />
          </li> 
      ))}
    </ul>
