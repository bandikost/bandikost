...
const skills = [
  { src: js, bg: "bg-yellow-100" },
  { src: rt, bg: "bg-blue-100" },
  { src: ui, bg: "bg-purple-100" },
  { src: axios, bg: "bg-green-100" },
  { src: jest, bg: "bg-pink-100" },
];
    ...
    <Suspense fallback={<Preloader />}>          
        <VantaBackground>
          ...
          <section className="relative w-full h-100 flex justify-end bg-gray-900">
                ...
                <div className="ml-10"><p className="text-size text-white">Используемые технологии:</p> 
                    <ul className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        {skills.map((skill, i) => (
                            <li key={i}>
                            <img
                                className="w-24 h-24 p-0 border border-gray-900 rounded-lg object-contain bg-yellow-400"
                                src={src}
                                alt={`skill-${i}`}
                            />
                            </li>
