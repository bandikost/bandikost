...
export default function CategoryFilms() {
    ...
    const initialFilms = films.filter(film => film.categoryId === categoryId)
    const [sortedFilms, setSortedFilms] = useState([])

    useEffect(() => {
        setSortedFilms(initialFilms)
    }, [id])

    const sortByName = () => {
        const sorted = [...sortedFilms].sort((a, b) => a.name.localeCompare(b.name) )
  ...
  if (!category) {
    ...
    <button onClick={sortByName}>Сортировать фильмы</button>
        <div className="mt-10 flex flex-col items-center justify-center">
        <ul className="space-y-6">
          {sortedFilms.map(film => (
            <li key={film.id} className="flex">
              <p className="p-1 md:text-xl flex flex-col">
                {film.name}:
                <span className="text-sm ml-4 mt-2 w-96">
                  {film.comment}:
                  <span className="text-xs ml-2 font-bold underline text-indigo-400">
                    {film.date}
                  </span>
                </span>
              </p>
