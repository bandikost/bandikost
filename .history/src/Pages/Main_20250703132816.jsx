...
useLayoutEffect(() => {
    ...
    <Suspense fallback={<Preloader />}>          
        ...
        <div className="container mx-auto px-4 md:flex md:justify-end items-center justify-center h-full">
                <div className="ml-5 md:ml-10"><p className="font-ubuntu text-lg sm:text-size text-white text-center sm:text-right pb-5 w-full">Используемые технологии: </p> 
                    <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 lg:grid-cols-6 xl:grid-cols-6 gap-4 w-48 sm:w-full">
                        {skills.map((skill, i) => (
                            <li key={i}>
                            <img
        ...
        <Suspense fallback={<Preloader />}>
            ...
            <div className="container mx-auto px-4 md:flex md:justify-start items-center h-full">
                <div><p className="text-sm text-center flex flex-col items-center justify-center font-ubuntu text-white pb-5">Проекты фриланса и для компаний, в которых я работал: </p> 
