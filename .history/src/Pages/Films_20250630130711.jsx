...
export default function Films() {
    ...
    const deferredFilms = films.slice(1, 4); // это фиктивно
    const recentFilms = films.slice(-5);

    const currentList = showSelected ? deferredFilms : recentFilms;
    ...
    <ul className="mt-20 md:text-2xl mx-10">
      ...
      {currentList.map((film) => (
          <li
            key={film.id}
            className="list-decimal m-3 border p-4 rounded-xl shadow text-center font-medium bg-white"
          >
            {film.name} — {film.date}
          </li>
        ))}
