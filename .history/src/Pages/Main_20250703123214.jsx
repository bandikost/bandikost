    ...
    <Suspense fallback={<Preloader />}>          
        ...
        <section className="relative w-full h-[380px] sm:h-[300px] center-flex bg-gray-900 md:flex md:justify-end">
            ...
            <section className="relative w-full h-120 flex bg-gray-900">
                ...
                <div className="ml-10"><p className="text-size text-white text-center pb-5">Проекты фриланса и для компаний, в которых я работал: </p> 
                    <ul className="grid grid-cols-3 gap-3">
                        {projects.map((proj) => (
                            <li className="text-white list-none border w-48 h-48" key={proj.id}>{proj.name}</li>
                        ))}
                    </ul>
                </div>
        </section>
