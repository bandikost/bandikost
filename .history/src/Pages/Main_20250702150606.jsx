...
useLayoutEffect(() => {
    ...
    <Suspense fallback={<Preloader />}>          
        ...
        <section className="relative w-full h-[300px]  sm:h-[400px] center-flex bg-gray-900 md:flex md:justify-end">

                <div className="ml-10 relative left-[-10px] sm:left-0"><p className="text-lg sm:text-size text-white text-center sm:text-right pb-5 w-full">Используемые технологии: </p> 
                    <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 lg:grid-cols-6 gap-4 ">
                        {skills.map((skill, i) => (
                            <li key={i}>
                            <img
                                className={`w-18 h-18 md:w-24 md:h-24 p-0 border border-gray-900 rounded-lg object-contain ${skill.bg}`}
                                src={skill.src}
                                alt={`skill-${i}`}
                            />
                            </li>
