...
export default function MainVideo() {
    ...
    return (
         <section className="relative w-full h-64 flex bg-gray-900 justify-center items-center md:justify-start">
                    <div className="flex flex-col justify-center items-center">
                        ...
                        <div className="ml-0 sm:ml-5 ">
                            <ul className="md:p-0 py-5 font-ubuntu text-size text-white flex items-center justify-center text-center md:items-start flex-col md:text-lg w-82 ">Сайт на React (CRA)
                                {codeForSPA.map((c) => (
                                    <li key={c.id} className="text-low-size w-64 md:ml-10 text-left p-1 m-0 sm:text-size-small-medium md:text-sm sm:w-full list-decimal ">{c.name}</li>
                                ))}
                            </ul>
