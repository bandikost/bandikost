    ...
    return (
         <section className="relative w-full h-120 flex bg-gray-900 justify-center items-center md:justify-start">
                    <div className="flex flex-col justify-center items-center">
                        ...
                        <div className="ml-5">
                            <ul className="font-ubuntu text-size text-white flex items-center justify-center text-center md:items-start flex-col md:text-lg sm:w-48 ">Сайт на React (CRA)
                                {codeForSPA.map((c) => (
                                    <li key={c.id} className="ml-10 text-left flex items-center justify-center p-1 m-0 text-size-small-medium md:text-sm w-full list-decimal ">{c.name}</li>
                                ))}
                            </ul>
