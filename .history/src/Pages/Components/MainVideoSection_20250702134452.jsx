         ...
         <section className="relative w-full h-120 flex bg-gray-900 justify-center items-center md:justify-start">
                    <div className="flex flex-col justify-center items-center">
                        ...
                        <div className="border border-indigo-300 rounded-xl ">
                           <video className="w-full max-w-[300px] h-auto sm:max-w-[400px] md:max-w-96 md:h-auto lg:max-w-96 border-0 sm:border sm:rounded-xl" src={code} autoPlay loop muted playsInline/>
                        </div>
                        <div className="ml-5">
                            <ul className="md:p-0 py-5 font-ubuntu text-size text-white flex items-center justify-center text-center md:items-start flex-col md:text-lg w-96 ">Сайт на React (CRA)
                                {codeForSPA.map((c) => (
                                    <li key={c.id} className="text-low-size ml-2 md:ml-10 text-left  p-1 m-0 sm:text-size-small-medium md:text-sm w-full list-decimal ">{c.name}</li>
                                ))}
                            </ul>
