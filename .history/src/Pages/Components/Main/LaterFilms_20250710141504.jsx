import { NavLink } from "react-router-dom"
import FilmPoster from "../MicroComponents/FilmPoster";
import { useState } from "react";

export default function LaterFilms({showSelected, deferredFilms, showFilms, recentFilms}) {

    const [ currentIndex, setCurrentIndex ] = useState(0);
    const cards = favorite;
    const length = cards.length;
  
    const getWrappedIndex = (index, length) => {
      return (index + length) % length;
    };
  
    const handleNext = () => {
      setCurrentIndex((prev) => (prev + 1) % length);
    };
  
    const handlePrev = () => {
      setCurrentIndex((prev) => (prev - 1 + length) % length);
    };
  
    const getVisibleCards = () => {
      return [
        showSelected[getWrappedIndex(currentIndex - 1, length)],
        showSelected[getWrappedIndex(currentIndex, length)],
        showSelected[getWrappedIndex(currentIndex + 1, length)],
      ];
    };
  
    if (currentIndex === undefined || currentIndex === null) return null; 
  
    const visibleCards = getVisibleCards();
 
    return (
        <>
         <h1 className="font-semibold text-xl md:text-xl my-10">
            {showSelected
          ? `${showFilms}  ${showFilms < 2 ? "фильм отложен" : "фильма отложенных"}: `
          : '3 последних фильма: '}
        </h1>
    <ul className="md:text-2xl mx-10 grid grid-cols-3">

      {(showSelected ? deferredFilms : recentFilms).map((film) => (

        <li key={film.id} className="flex items-center flex-col mx-2 pt-2 text-size-small-medium ">
          <NavLink className="flex flex-col items-center" to={`/category/${film.categoryId}`}>
            <FilmPoster className="w-40 hover:opacity-70 transition-colors duration-300 cursor-pointer border rounded-xl shadow-[5px_5px_10px_rgba(0,0,0,0.2)]" filmName={film.name} />
            <p className="text-ideal-phone">{film.date}</p>
          </NavLink>
        </li>

      ))}
    </ul>
    </>
    )
}