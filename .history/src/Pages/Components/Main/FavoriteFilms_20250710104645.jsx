...
export default function FavoriteFilms() {
  ...
  const visibleCards = getVisibleCards();
    ...
    return (
      ...
      <button className='hover:bg-white transition duration-200 border- border-2 border-slate-100 rounded-lg' 
          style={{margin: '15px', padding: "5px 10px"}} onClick={handlePrev}>← Назад</button>
          <button className='hover:bg-white transition duration-200 border- hover:border-2 border-slate-100 rounded-lg' 
          style={{margin: '15px', padding: "5px 10px"}} onClick={handleNext}>Вперёд →</button>
    </ul>
    </>
    )
}