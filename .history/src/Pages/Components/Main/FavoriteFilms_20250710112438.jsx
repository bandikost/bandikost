...
export default function FavoriteFilms() {
  ...
  const visibleCards = getVisibleCards();
   ...
   return (
    <div>
      {favorite.map(mainfav => (
<FilmPoster key={mainfav.id}
              className="w-48 hover:opacity-90 transition duration-300 cursor-pointer border rounded-xl shadow-[8px_8px_10px_rgba(0,0,0,0.3)]"
              filmName={mainfav.name}
              year={mainfav.year}>
