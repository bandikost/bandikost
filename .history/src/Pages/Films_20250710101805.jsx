...
export default function Films() {
    ...
    <h1 className=" font-bold text-xl md:text-3xl">Избранные:</h1>
      <ul className="mt-10 md:text-2xl mx-10 grid grid-cols-4 ">
        {favorite.map((fav) => (
            <li key={fav.id} className="flex items-center flex-col mx-3 pt-2font-normal 
               text-center bg-white space-y-2 text-size-small-medium hover:bg-indigo-100 transition-colors duration-300 cursor-pointer">
            <FilmPoster className="w-40  border rounded-xl shadow-[5px_5px_10px_rgba(0,0,0,0.2)]" filmName={fav.name} year={fav.year} />
            {fav.name}
          </li> 
      ))}
    </ul>
    ...
    <ul className="mt-10 md:text-2xl mx-10 grid grid-cols-3">
      ...
      {(showSelected ? deferredFilms : recentFilms).map((film) => (
        ...
        <li key={film.id} className="flex items-center flex-col m-3 border p-4 rounded-xl font-normal 
          ...
          <FilmPoster className="w-40" filmName={film.name} />
        </li>
