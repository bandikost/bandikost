    ...
    <Suspense fallback={<Preloader />}>          
        ...
        <section className="relative w-full h-[380px] sm:h-[260px] center-flex bg-gray-900 md:flex md:justify-end">
                ...
                <div className="ml-10"><p className="text-lg sm:text-size text-white text-center sm:text-right pb-5 w-full">Используемые технологии: </p> 
                    <ul className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-6 xl:grid-cols-6 gap-4 ">
                        {skills.map((skill, i) => (
                            <li key={i}>
                            <img
                                className={`w-16 sm:w-16 md:w-20 lg:w-24 xl:w-28 h-12 sm:h-16 md:h-20 lg:h-24 xl:h-28 p-0 border border-gray-900 rounded-lg object-containt ${skill.bg}`}
                                src={skill.src}
                                alt={`skill-${i}`}
                            />
                            </li>
