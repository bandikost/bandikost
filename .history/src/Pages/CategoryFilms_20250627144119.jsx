              ...
              {groupedFilms[letter].map(film => (
                <li key={film.id} className="text-sm flex mb-8">
                  <p className='p-1 md:text-xl flex flex-col'>{film.name}: 
                        <span className='text-sm ml-4 mt-2 w-96 sm:w-72'>{film.comment}: <span className='text-xs ml-2 font-bold transform underline text-indigo-400'>{film.date}</span></span>
                    </p>
