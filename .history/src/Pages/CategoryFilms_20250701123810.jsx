              ...
              <ul className="space-y-2">
                {groupedFilms[letter].map(film => (
                  <li key={film.id} className="text-sm flex mb-8">
                    <p className='p-1 md:text-xl flex flex-col font-bold'>{film.name}<Link target='_blank' to={film.link} className='text-sm ml-4 mt-2 w-56 sm:w-96 font-light text-blue-400 underline hover:text-blue-700'>Перейти</Link></p>
                    <p className='text-sm ml-4 mt-2 w-56 sm:w-96 font-light'>{film.comment}: <span className='text-xs ml-2 font-bold transform underline text-indigo-400'>{film.date}</span></p>
