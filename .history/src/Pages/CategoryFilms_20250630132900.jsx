...
export default function CategoryFilms() {
    ...
    return films.filter(film => 
      film.name.toLowerCase().startsWith(debouncedValue.toLowerCase())
  );
}, [searchValue]);
  ...
  const sortedLetters = Object.keys(groupedFilms).sort((a, b) => a.localeCompare(b))
    ...
    <form className='flex items-center justify-center'>
        <label className='flex items-center'>
            <input className='mx-3 p-1 border rounded-xl pl-2' value={searchValue} onChange={(e) => setSearchValue(e.target.value)} placeholder='Поиск по названию' />
        </label>
    </form>
