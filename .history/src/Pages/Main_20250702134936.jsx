       ...
       {/*  <VantaBackground>
        ...
        <Slasher />  
          ...
          <section className="relative w-full h-[600px] sm:h-[400px] center-flex bg-gray-900 md:flex md:justify-end">
                ...
                <div className="ml-10 flex flex-col sm:flex-row items-center"><p className="text-lg sm:text-size text-white text-center sm:text-right pb-5 w-96">Используемые технологии: </p> 
                    <ul className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-6 lg:grid-cols-6 gap-4 h-[500px] flex items-center justify-center ">
                        {skills.map((skill, i) => (
                            <li key={i}>
                            <img
                                className={`w-16 h-16 md:w-24 md:h-24 p-0 border border-gray-900 rounded-lg object-contain ${skill.bg}`}
                                src={skill.src}
                                alt={`skill-${i}`}
                            />
                            </li>
