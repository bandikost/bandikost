...
useLayoutEffect(() => {
    ...
    <Suspense fallback={<Preloader />}>          
        ...
        <Slasher />  
          ...
          <section className="relative w-full h-[400px] flex  bg-gray-900 md:center-flex">

                <div className="ml-10"><p className="text-size text-white text-left pb-5 md:center-flex">Используемые технологии: </p> 
                    <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 md:center-flex">
                        {skills.map((skill, i) => (
                            <li key={i}>
                            <img
                                className={`w-16 h-16 p-0 border border-gray-900 rounded-lg object-contain ${skill.bg}`}
                                src={skill.src}
                                alt={`skill-${i}`}
                            />
                            </li>
