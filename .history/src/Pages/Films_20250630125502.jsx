...
export default function Films() {
    ...
    <h2 className="font-bold text-xl md:text-3xl">Категории фильмов</h2>
        ...
        <ul className="mt-20 md:text-2xl grid grid-cols-2">
            Последние 5 добавленных фильма: 
            {films.slice(-5).map(film => (
                <li key={film.id} className="list-decimal border p-4 rounded-xl shadow text-center font-medium bg-white space-y-2">{film.name} - {film.rate}</li>
            ))}
        </ul>
    </div>
