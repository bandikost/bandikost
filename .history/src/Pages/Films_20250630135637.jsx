...
export default function Films() {
    ...
    <ul className="mt-10 md:text-2xl mx-10">
      ...
      {(showSelected ? deferredFilms : recentFilms).map((film) => (
        ...
        <li
          key={film.id}
          className="list-decimal m-3 border p-4 rounded-xl font-normal 
          shadow-[5px_5px_10px_rgba(0,0,0,0.2)] text-center bg-white space-y-2 text-size-small-medium" 
        >
            
          <NavLink className="hover:bg-grey-900" to={`/category/${film.categoryId}`}>{film.name} {film.date >= "" ? `- ${film.date}`: ``}</NavLink>
        </li>
        </div>
      ))}
    </ul>
