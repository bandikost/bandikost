...
useLayoutEffect(() => {
    ...
    <Suspense fallback={<Preloader />}>          
        ...
        <section className="relative w-full h-[380px] sm:h-[300px] center-flex bg-gray-900 md:flex md:justify-end">
            ...
            <section className="relative w-full h-120 flex bg-gray-900">
                ...
                <div className="ml-10"><p className="text-size font-ubuntu text-white pb-5">Проекты фриланса и для компаний, в которых я работал: </p> 
                    <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                        {projects.map((proj) => (
                            <li className="text-white flex flex-col justify-between list-none  font-ubuntu text-ideal-phone sm:text-sm md:text-size-small-medium" key={proj.id}>
                                <p className="border rounded-xl p-5 md:h-40 md:w-40 w-36 h-36 mt-5 mb-1">{proj.name}</p>
                                 <NavLink to={`work/${proj.id}`} className="text-low-size  text-white mr-10 sm:mr-1 md:mr-5">→ <span className="transform duration-100 underline hover:text-indigo-300">Перейти в раздел</span></NavLink>
                            </li>
