...
export default function CategoryFilms() {
 ...
 {searchValue.length > 0 ? (
  <div className="flex justify-center items-start px-4">
    <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 m-4 w-full max-w-6xl">
      {filteredFilms.slice(-5).map((film) => (
        <li
          key={film.id}
          className="border p-3 rounded shadow text-center font-medium bg-white"
        >
          {film.name} - {film.rate} 
          <br></br>
          <span>
