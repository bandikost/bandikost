...
useLayoutEffect(() => {
    ...
    <Suspense fallback={<Preloader />}>          
        ...
        <section className="relative w-full h-[380px] sm:h-[260px] center-flex bg-gray-900 md:flex md:justify-end">

                <div className="ml-5 md:ml-10"><p className="text-lg sm:text-size text-white text-center sm:text-right pb-5 w-full">Используемые технологии: </p> 
                    <ul className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-6 xl:grid-cols-6 gap-4 w-72 sm:w-full">
                        {skills.map((skill, i) => (
                            <li key={i}>
                            <img
