...
export default function CategoryFilms() {
    ...
    const filmFinder = useRef(null)
    const [valueName, setValueName] = useState()

    const handleSearch = (e) => {
      if(filmFinder === films.name) {
  ...
  const sortedLetters = Object.keys(groupedFilms).sort((a, b) => a.localeCompare(b))
    ...
    <form className='flex items-center justify-center'>
      <label className='flex items-center m-5'>
            <input className='mx-3 p-1 border rounded-xl pl-2' placeholder='Поиск по букве' />
            <button onClick={handleSearch} className='border-gray-900 border px-3 py-1 rounded-xl hover:bg-gray-900 hover:text-white transform duration-300'>Поиск</button>
        </label>
        <label className='flex items-center'>
            <input className='mx-3 p-1 border rounded-xl pl-2' ref={filmFinder} onChange={(e) => e.target.value} placeholder='Поиск по названию' />
            <button className='border-gray-900 border px-3 py-1 rounded-xl hover:bg-gray-900 hover:text-white transform duration-300'>Поиск</button>
        </label>
    </form>
